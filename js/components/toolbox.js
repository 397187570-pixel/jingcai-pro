 ...
 */
  function renderToolbox() {
    ...
    /* 统计近 30 天有结果的记录 */
      ...
      const wins = recent.filter(function(r) { return r.result === 'win'; }).length;
    ...
    const totalStake = settled.reduce((s, r) => s + (Number(r.amt) || 0), 0);
    const rate = settled.length ? Math.round(winCount / settled.length * 100) : 0;
    const roi = totalStake ? (totalProfit / totalStake * 100) : 0;
    const valueWins = records.filter(r => r.result === 'win' && r.valueBet).length;

    setText('toolboxTotal', records.length);
    setText('toolboxWin', winCount);
    setText('toolboxLoss', lossCount);
    setText('toolboxRate', settled.length ? rate + '%' : '--');
    setText('toolboxWinLoss', winCount + ' 中 / ' + lossCount + ' 未');
    setText('toolboxRoi', settled.length ? (roi > 0 ? '+' : '') + roi.toFixed(1) + '%' : '--');
    setText('toolboxValueWin', valueWins);
  }
