...
/* 依赖解析：浏览器用全局，Node 用 require */
  let esc, deVigOdds, eloPredict, findValueBets, CONFIG, calibrateProbability, getOddsForCell;
    ...
    const cards = matches.map((m, idx) => {
      ...
      if (haveModel && odds.h && odds.h > 1) {
        ...
        if (window.MultiDim && r.directionProbs && r.directionProbs.length === 3) {
          md = window.MultiDim.buildMultiDim(
            { homeWin: r.directionProbs[0], draw: r.directionProbs[1], awayWin: r.directionProbs[2] },
            { line: goalLine }
          );
          topOpt = md.top5 && md.top5[0] ? Object.assign({}, md.top5[0], { odds: getOddsForCell ? getOddsForCell(md.top5[0], m) : null }) : null;
          multiDimHtml = renderMultiDimBoard(md, idx, m);
        }

        const topClass = topOpt ? (topOpt.key === '1x2-home' || topOpt.key === 'ah-home' || topOpt.dim === '进球数' ? 'badge-green' : (topOpt.key === '1x2-away' || topOpt.key === 'ah-away' ? 'badge-red' : 'badge-amber')) : dirClass;
        const topLabel = topOpt ? topOpt.label : r.directionZh;
        const topOdds = topOpt && topOpt.odds ? topOpt.odds.toFixed(2) : (r.oddsImplied[['home','draw','away'].indexOf(r.direction)]).toFixed(2);
        const topPct = topOpt ? (topOpt.p * 100).toFixed(1) + '%' : confPct + '%';
      ...
      <div class="rec-card" style="padding:14px;border:1px solid var(--border-color);border-radius:10px;margin-bottom:12px;background:var(--bg-card);">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
          ...
          <div style="text-align:right;min-width:72px;">
            <span class="badge ${topClass}" style="font-size:13px;padding:6px 10px;">✓ ${esc(topLabel)}</span>
            <div style="font-size:11px;color:var(--text-muted);margin-top:4px;font-family:var(--font-mono);">@ ${topOdds}</div>
            <div style="font-size:10px;color:var(--text-muted);">${topPct}</div>
          </div>
        </div>
  ...
  /* ============================================
   ...
   ============================================ */
  function renderMultiDimBoard(md, idx, m) {
    ...
    const cellOdds = function (c) { return getOddsForCell ? getOddsForCell(c, m) : null; };
    ...
    const ahLabel = '让球 ' + md.lineLabel + (md.lineSuggested ? '（模型建议）' : '');
    const ahBars = bars([
    ...
    const top5Html = md.top5.map(function (c, i) {
      ...
      '<span style="font-family:var(--font-mono);font-size:12px;color:var(--text-muted);white-space:nowrap;">' + esc(oddTxt) + '</span>' +
      '<span style="font-family:var(--font-mono);font-size:13px;color:' + col + ';font-weight:700;min-width:48px;text-align:right;">' + pct(c.p) + '</span>' +
      '</div>';
    }).join('');
    ...
    try {
      ...
      /* API 调用成功但返回 0 场：给出精准诊断 */
        ...
        if (diag.detectedLeagues && diag.detectedLeagues.length) {
          ...
          if (diag.deadKeys && diag.deadKeys.length) {
            detailHtml += '🚫 本会话已确认 404/422 的 sport key（不再重试）：' + esc(diag.deadKeys.join('、')) + '<br>';
          }
          if (diag.perKey && Object.keys(diag.perKey).length) {
            detailHtml += '各 key 返回场数：';
            const pkParts = [];
            Object.keys(diag.perKey).forEach(function(k) {
              const v = diag.perKey[k];
              const label = k.replace(/^broad:/, ' broad-');
              if (v && v.ok) {
      ...
      const matched = (window.OddsApi && window.OddsApi.matchByTime)
      ...
      if (!matched.length) {
        jcMatches.forEach(function(m) {
          ...
          const match = intOdds.find(function(i) {
      ...
      // 未匹配诊断：展示国际赔率返回的联赛分布、时间范围、样本队名
        ...
        intOdds.forEach(function(io, idx) {
          const sk = io.sportKey || '未知';
          sportCounts[sk] = (sportCounts[sk] || 0) + 1;
          if (io.commence) {
            const t = new Date(io.commence).getTime();
            if (!earliest || t < earliest) earliest = t;
            if (!latest || t > latest) latest = t;
          }
          if (idx < 5) samples.push((io.homeTeam || '?') + ' vs ' + (io.awayTeam || '?'));
        });
      /* 别名映射明细：哪些竞彩队已识别（含英文化）、哪些未识别 */
        let aliasedTeams = 0, missingTeams = [];
        if (window.OddsApi && window.OddsApi.resolveTeamAlias) {
          jcMatches.forEach(function(m) {
            const a1 = window.OddsApi.resolveTeamAlias(m.homeTeam, m.league);
            const a2 = window.OddsApi.resolveTeamAlias(m.awayTeam, m.league);
            if (a1) aliasedTeams++; else missingTeams.push(m.homeTeam + '（' + (m.league || '?') + '）');
            if (a2) aliasedTeams++; else missingTeams.push(m.awayTeam + '（' + (m.league || '?') + '）');
          });
        }
        const jcTimes = [];
        jcMatches.forEach(function(m) {
        ...
        const diag = result.diagnosis || {};
        let detail = '<div style="margin-top:6px;font-size:11px;color:var(--text-muted);">';
        detail += '国际赔率联赛分布：<br>';
        ...
        if (window.OddsApi && window.OddsApi.resolveTeamAlias) {
          detail += '别名桥接：' + aliasedTeams + ' 队已识别英文';
          if (missingTeams.length) {
            detail += '；' + missingTeams.length + ' 队未识别：' + esc(missingTeams.slice(0, 6).join('、')) + (missingTeams.length > 6 ? ' 等' : '');
          }
          detail += '<br>';
        }
        if (diag.unmappedLeagues && diag.unmappedLeagues.length) {
      ...
      matched.forEach(function(item) {
        const m = item.jcMatch;
        const intOdd = item.intOdd;
        const odds = m.odds;
        if (!odds || !odds.h) return;
  ...
  function renderQuotaBar(quota) {
    const el = document.getElementById('oddsQuotaBar');
    if (!el) return;
    ...
    const color = pct < 60 ? 'var(--c-green)' : pct < 85 ? 'var(--c-amber)' : 'var(--c-red)';
