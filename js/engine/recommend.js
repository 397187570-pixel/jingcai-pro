...
(function (global) {
   ...
   *  league 优先用本联赛历史; 无则 ALL 兜底。无 euGap 时退化为旧版 +2% 软阈值。
   */
  function valueSignal(jc, eu, league, euGap) {
    if (!eu) return null;
    ...
    for (var i = 0; i < 3; i++) {
      var g = Math.round((pj[i] - pe[i]) * 10000) / 10000;
      sig[labels[i]] = g;
      var st = gapStats(euGap, league, labels[i]);
      if (st && st.std > 0) {
        z[labels[i]] = Math.round(((g - st.mean) / st.std) * 100) / 100;
        pct[labels[i]] = Math.round(normCdf((g - st.mean) / st.std) * 1000) / 10; // %
        flagged[labels[i]] = g > st.p90; // 超过历史 90 分位 => 显著价值
      } else {
        z[labels[i]] = null; pct[labels[i]] = null;
        flagged[labels[i]] = g > 0.02; // 无先验: 旧软阈值
      }
    }
    var best = labels[0];
    for (var j = 1; j < 3; j++) if (sig[labels[j]] > sig[best]) best = labels[j];
    return {
      signal: sig, zscore: z, percentile: pct, flagged: flagged,
      bestValueSide: best, bestValue: sig[best], priorSource: (euGap ? 'history' : 'none')
    };
  }
   ...
   */
  function recommend(m, calib, leagueCal, asian, euGap) {
    var jc = m.jc;
    ...
    // 方向 = 校准后概率最高者
    var dir = 0;
    for (var i = 1; i < 3; i++) if (conf[i] > conf[dir]) dir = i;
    ...
    // 价值信号
    var vs = valueSignal(jc, m.eu, m.league, euGap);

    // 文字建议
    var rec = zh[dir] + ' (校准置信度 ' + (conf[dir] * 100).toFixed(1) + '%)';
    ...
    if (vs && vs.flagged[vs.bestValueSide]) {
      ...
      rec += ' | 价值信号: ' + side + ' 竞彩优于欧盘 +' + (vs.bestValue * 100).toFixed(1) +
             '% (' + pctTxt + ' ' + zTxt + ', 突破历史 p90)';
    }
    ...
    return {
      ...
      directionConfidence: Math.round(conf[dir] * 10000) / 10000,
