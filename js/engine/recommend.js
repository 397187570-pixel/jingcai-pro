...
(function (global) {
  ...
  function calibrate(conf, p) {
    ...
    let best = p;
    for (let i = 0; i < conf.table.length; i++) {
      const rg = conf.table[i].range;
      let lo, hi;
  ...
  const SIDE_KEY = { home: 'jh', draw: 'jd', away: 'ja' };
  const MIN_GAP_N = 40;
  function gapStats(euGap, league, side) {
    const k = SIDE_KEY[side];
    if (!k) return null;
    const L = euGap && euGap[league] && euGap[league][k];
    const A = euGap && euGap['ALL'] && euGap['ALL'][k];
  ...
  function valueSignal(jc, eu, league, euGap, conf) {
    const pj = implied(jc[0], jc[1], jc[2]);
    ...
    if (eu) {
      const pe = implied(eu[0], eu[1], eu[2]);
      const sig = {}, z = {}, pct = {}, flagged = {};
      for (let i = 0; i < 3; i++) {
        const g = Math.round((pe[i] - pj[i]) * 10000) / 10000; // V = 欧盘 − 竞彩
        sig[labels[i]] = g;
        const st = gapStats(euGap, league, labels[i]);
        if (st && st.std > 0) {
          const thr = (typeof st.p10 === 'number') ? st.p10 : -st.p90;
      ...
      let best = labels[0];
      for (let j = 1; j < 3; j++) if (sig[labels[j]] > sig[best]) best = labels[j];
      return {
    ...
    if (!conf) return null;
    const edge = {}, z = {}, pct = {}, flagged = {}, strong = {};
    for (let k = 0; k < 3; k++) {
      const e = Math.round((conf[k] - pj[k]) * 10000) / 10000; // 模型概率 - 市场隐含
      edge[labels[k]] = e;
      const st2 = gapStats(euGap, league, labels[k]);
      if (st2 && st2.std > 0) {
    ...
    let best2 = labels[0];
    for (let m2 = 1; m2 < 3; m2++) if (edge[labels[m2]] > edge[best2]) best2 = labels[m2];
    return {
  ...
  function recommend(m, calib, leagueCal, asian, euGap, opts) {
    ...
    const blendWeight = (typeof opts.blendWeight === 'number') ? opts.blendWeight : null;
    const jc = m.jc;
    const pi = implied(jc[0], jc[1], jc[2]);
    const labels = ['home', 'draw', 'away'];
    const zh = ['主胜', '平', '客胜'];

    // 校准后置信度(竞彩)
    const conf = pi.map(function (p) { return calibrate(calib, p); });

    // 方向概率: 默认竞彩校准; 提供欧盘混合权重时融合欧盘去水概率(公平概率)
    let pDir = conf;
    if (blendWeight !== null && m.eu) {
      const pe = implied(m.eu[0], m.eu[1], m.eu[2]); // 欧盘去水隐含(公平概率)
      pDir = conf.map(function (c, i) { return blendWeight * c + (1 - blendWeight) * pe[i]; });
    }

    // 方向 = 方向概率最高者
    let dir = 0;
    for (let i = 1; i < 3; i++) if (pDir[i] > pDir[dir]) dir = i;

    // 联赛假设(仅供参考)
    let lgEdge = null;
    if (leagueCal && leagueCal.leagues && leagueCal.leagues[m.league]) {
      const L = leagueCal.leagues[m.league];
    ...
    if (asian && asian.handicaps && m.goalLine !== null) {
      const key = (Math.round(m.goalLine * 10) / 10).toFixed(1);
      if (asian.handicaps[key]) asianInfo = { line: m.goalLine, dist: asian.handicaps[key] };
    }

    // 价值信号(无实时欧盘时走模型价值边际, 欧盘历史先验作标尺)
    const vs = valueSignal(jc, m.eu, m.league, euGap, conf);

    // 文字建议
    let rec = zh[dir] + ' (校准置信度 ' + (pDir[dir] * 100).toFixed(1) + '%)';
    const caution = '竞彩固定抽水约 12.9%, 长期为负 EV; 本建议为概率参考, 非稳赚。';
    if (vs && vs.flagged[vs.bestValueSide]) {
      const side = vs.bestValueSide;
      const sideZh = zh[labels.indexOf(side)];
      const pctTxt = vs.percentile && vs.percentile[side] !== null && vs.percentile[side] !== undefined ? ('历史分位 ' + vs.percentile[side] + '%') : '';
      const zTxt = vs.zscore && vs.zscore[side] !== null && vs.zscore[side] !== undefined ? ('z=' + vs.zscore[side]) : '';
      if (vs.euFree) {
        // 路径2: 模型价值(关键-free)
        const strongTag = (vs.strong && vs.strong[side]) ? '💎💎 超欧盘历史 p90' : '💎 模型价值';
  ...
  const api = { recommend: recommend, implied: implied, calibrate: calibrate, valueSignal: valueSignal, normCdf: normCdf };
